...
AIGC:
  ...
  Label: '1'
  ProduceID: '35e5f7e9-8128-47bc-8d02-f2a7ecf5854e'
  PropagateID: '35e5f7e9-8128-47bc-8d02-f2a7ecf5854e'
  ReservedCode1: 'e8dfca26-c8e2-4d48-b7b2-2fd97b1d7621'
  ReservedCode2: 'e8dfca26-c8e2-4d48-b7b2-2fd97b1d7621'
---
...
| Round 6 | 字节统计埋点 + SSE代理 + 遗留BUG | 5 | TotalBytes 统计 + SSE Send/Data 代理 + cWinSock Exists + cTlsSocket 死锁 |
| **合计** | | **17+1 文件** | **BUG-1~10, 15（12 个 BUG）+ 42 处 As New + Statistics + R6** |

---

## Round 6：字节统计埋点 + SSE 代理补全 + 遗留 BUG 修复

### 1. TotalBytesReceived / TotalBytesSent 埋点

`cHttpServerStatistics` 的 `TotalBytesReceived` 和 `TotalBytesSent` 字段此前已声明但未接入数据。本轮补全三个埋点：

| 文件 | 位置 | 埋点内容 |
|------|------|---------|
| `cHttpServer.cls` | OnDataArrival，GetData 后 | `Statistics.TotalBytesReceived += UBound(newData) + 1` |
| `cHttpServerResponse.cls` | SendBodyByte，SendData 后 | `Client.Parent.Statistics.TotalBytesSent += UBound(Data) + 1` |
| `cHttpServerResponse.cls` | SendHeader，SendData 后 | `Client.Parent.Statistics.TotalBytesSent += Len(Head) + 2` |

### 2. cSSEContext 补充 Send / Data 代理

控制器中仅能通过 `ctx.SSE`（cSSEContext）访问 SSE 功能，但原代理缺少核心的 `Send` 方法和 `Data` 字典：

```vb
' 新增：代理 cSSE.Data 字典
Public Property Get Data() As Scripting.Dictionary
    Set Data = SSEInst.Data
End Property

' 新增：代理 cSSE.Send 方法
Public Function Send(Optional ByVal hSocket As Long) As Boolean
    Send = SSEInst.Send(hSocket)
End Function
```

### 3. cWinSock.cls RemoveClient 隐性 BUG（BUG-16）

**级别**: 高危（隐性）

**症状**: 客户端断开连接后从 `m_cClients` 集合中移除失败，客户端对象持续累积。

**根因**: `RemoveClient` 第 1836 行调用 `m_cClients.Exists(oClient.Tag)`，但 `m_cClients` 声明为 `Collection`（第 157 行），VB6 的 Collection **没有 Exists 方法**。`On Error Resume Next` 静默吞掉 Error 438，If 条件求值为 False，整个移除逻辑被跳过。

**修复**: 替换为 VB6 Collection 惯用模式：
...
' 修复后
Dim oTmp As cWinsock
Set oTmp = m_cClients(oClient.Tag)
If Not oTmp Is Nothing Then
```

### 4. cTlsSocket TLS 握手 FD_WRITE 死锁（BUG-17）

**级别**: 高危（条件触发）

**症状**: TLS 连接在特定条件下（高延迟/小缓冲区/大证书链）握手超时失败。

**根因**: `m_oAsyncSocket_OnSend` 第 1811 行在 `TlsIsReady=False`（握手未完成）时，直接 `GoTo QH` 丢弃了 `FD_WRITE` 事件。如果 `pvHandleSend` 之前因 `WSAEWOULDBLOCK` 退出（发送缓冲区残留数据），后续再无机会发送，导致握手死锁。

**修复**: 在丢弃 FD_WRITE 前，先检查并刷新发送缓冲区：

```vb
If Not TlsIsReady(m_uCtx) Then
    ' BUG修复：TLS握手期间若有发送缓冲区残留数据，利用FD_WRITE刷新
...
文档 `context.md` 和 `sse-server.md` 中存在大量与实际 API 不符的方法描述：

| 错误 | 修正 |
|------|------|
| `ctx.SSE.Send(hSocket, "event", "data", "id")` — 4参数 | `ctx.SSE.SendPack "event", "data", hSocket` |
| `ctx.SSE.Broadcast(...)` — 不存在 | `ctx.SSE.SendPack "action", "content"`（无 hSocket 时广播） |
| `ctx.SSE.IsConnected(hSocket)` — 不存在 | `ctx.SSE.ClientCount > 0` |
| `Dim sse As cSSE` — 类型错误 | `Dim sse As cSSEContext`（ctx.SSE 返回 cSSEContext） |
| ToUser/ToGroup 返回 `cSSE` | 返回 `cSSEContext` |
| Start 方法出现在 ctx.SSE API 中 | Start 属于 cSSE 类，cSSEContext 无此方法 |

### Round 6 修改文件清单

```
src\HttpServer\cHttpServer.cls            ← TotalBytesReceived 埋点
src\HttpServer\cHttpServerResponse.cls    ← TotalBytesSent 埋点（2处）
src\SSE\cSSEContext.cls                   ← 补充 Send + Data 代理
src\Winsock\cWinSock.cls                 ← BUG-16 RemoveClient Exists 修复
src\Socket\cTlsSocket.cls                ← BUG-17 TLS 握手 FD_WRITE 死锁修复
docs\httpserver\context.md               ← SSE 文档修正
docs\httpserver\sse-server.md            ← SSE 文档修正
docs\redis\README.md                     ← 添加 testing.md 链接
```