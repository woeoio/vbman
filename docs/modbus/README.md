...
#### v1.1.0 (2026-01-16)
- ? 新增 `BindAddress` 属性支持（从站）- 可配置监听地址（127.0.0.1、0.0.0.0 或指定 IP）
- ? 更新枚举命名规范 - 添加 Master/Slave 后缀区分（如 `ModbusMasterFunctionCode`/`ModbusSlaveFunctionCode`）
- ? 优化方法命名 - `Stop()` 更名为 `StopMe()` 避免与 VB 关键字冲突
- ? 更新结构体命名 - `ModbusDefaults` 分为 `ModbusMasterDefaults` 和 `ModbusSlaveDefaults`
- ? 更新所有示例代码和文档以反映上述变更

#### v1.0.0 (2026-01-16)
- 初始版本发布
- 支持 Modbus RTU 和 TCP 双协议
- 完整功能码支持（0x01-0x10, 0x16, 0x17）
- 事件驱动模型
- 内置数据存储区
