...
Public Enum QRCodegenEcc
    QRCodegenEcc_LOW = 0                                                        ' QR码可以容忍大约7%的错误码字
    QRCodegenEcc_MEDIUM                                                         ' QR码可以容忍大约15%的错误码字
    QRCodegenEcc_QUARTILE                                                       ' QR码可以容忍大约25%的错误码字
    QRCodegenEcc_HIGH                                                           ' QR码可以容忍大约30%的错误码字
End Enum
...
Private LNG_STEP                                     As Long                    ' Saturnian编辑
...
Public Function QRCodegenBarcode(TextOrByteArray As Variant, _
    ...
    Optional ByVal BoostEcl As Boolean = True, _
    Optional PicSize As Long = 500) As StdPicture                               ' Saturnian编辑
    Dim baQrCode()      As Byte
    LNG_STEP = PicSize
    If LNG_STEP < 1 Then LNG_STEP = 1 Else If LNG_STEP > 500 Then LNG_STEP = 500
...
Public Function QRCodegenEncodeSegments(uSegments() As QRCodegenSegment, baQrCode() As Byte, _
    ...
    If Mask = QRCodegenMask_AUTO Then
        ...
        For lIdx = QRCodegenMask_0 To QRCodegenMask_7
            ...
            End If
            pvApplyMask baTemp, baQrCode, lIdx                                  '--- 由于XOR而撤消掩码
        Next
    End If
...
Public Function QRCodegenConvertToPicture(baQrCode() As Byte, _
    ...
    'Const LNG_STEP      As Long = 500 ' Edited by Saturnian
    ...
    End With
    hDC = 0
    '--- 填写魔法图片GUID{7BF80980-BF32-101A-8BBB-00AA00300CAB}
    aGUID(0) = &H7BF80980
    aGUID(1) = &H101ABF32
    aGUID(2) = &HAA00BB8B
    aGUID(3) = &HAB0C3000
